...
LLM_API_KEY = "msNIXWm2OZ0esZYeQ6xrwClgCO85dz39"  # <-- PUT YOUR API KEY HERE

# Model to use (leave empty for default, or specify like "gpt-4o", "claude-3-5-sonnet-20241022", etc.)
LLM_MODEL = "mistral-small-latest"  # <-- Optional: specify model or leave empty for default
...
class GeminiProvider(LLMProvider):
    ...
    def complete(self, prompt: str, system: str = None) -> str:
        ...
        # Some keys/regions don't expose every Gemini model slug. If the
        # configured model returns 404, retry a short fallback list.
        model_candidates = [
            self.model,
        ...
        for model_name in model_candidates:
            ...
            for attempt in range(4):
                try:
                    ...
                    # 404 likely means model slug unavailable; try next model.
                    if e.code == 404:
                        last_error = e
                        break
                    # 429 means quota/rate throttling; retry with backoff.
        ...
        if last_error:
            raise last_error
        raise RuntimeError("Gemini request failed with no fallback candidates.")
